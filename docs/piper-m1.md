...
M2a 已在同一后端增加 TCP 位姿 IK、轨迹和碰撞预检，详见 [M2a 报告](piper-m2a.md)。
本页的运动验收不代表物理抓放、CaP 或真机已完成。
...
M1 交付时的 `move_to(Pose)` 曾明确返回尚未实现 IK；当前该接口已由 M2a 实现，
但仍只代表 TCP 位姿运动能力，不代表物理抓放。
...
4. M2：在 M2a IK/避碰基础上完成已知 state 坐标物理抓放，记录成功及失败，并验证释放、撤离、稳定落入容器。
